...
export const login = async (req, res) => {
  ...
  try {
    ...
    if(passwordMatch){
        ...
        const token = jwt.sign({_id, username}, process.env.JWT_SECRET, {expiresIn: '24h'});
        return res.status(200).json({message: 'Login Successful...!', user: token, username: user.username})
    }else{
...
export const updateUser = async (req,res) => {
    ...
    try { 
        ...
        if(user._id.toString()!== req.params.id){
            return res.status(401).send({error: "You are not authorized to update this user"});
        }
        //update the user
